...
import InvitroItem from "../InvitroItem/index.jsx";
import "./style.css";
...
function Invitro() {
  ...
  const firstRun = async () => {
    ...
    setItemCount(invitroList.length);
    setLoading(false);
    setData(invitroList);
  };

  useEffect(() => {
    setCategory("In Vitro");
    firstRun();
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    await setDoc(doc(db, "invitro", id), {
      id,
      title,
      videoLink: link,
      videoId,
    ...
    <div className="section-container">
      <div className="grid-wrapper">
        {data &&
          data.map((item) => {
            return (
              <InvitroItem data={item} firstRun={firstRun} key={item.id} />
            );
          })}
      </div>
      {showModal && (
